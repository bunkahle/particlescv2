...
cdef int DRAWTYPE_POINT = 0
cdef int DRAWTYPE_CIRCLE = 1
cdef int DRAWTYPE_LINE = 2
cdef int DRAWTYPE_SCALELINE = 3
cdef int DRAWTYPE_BUBBLE = 4
cdef int DRAWTYPE_IMAGE = 5
# Gravity constants
cdef float UNIVERSAL_CONSTANT_OF_MAKE_GRAVITY_LESS_STUPIDLY_SMALL = 1000.0 
cdef float MAXDIST = 20.0
# Interpolation type constants
cdef int INTERPOLATIONTYPE_LINEAR = 200
cdef int INTERPOLATIONTYPE_COSINE = 201

cdef float PYIGNITION_VERSION = 1.0
cdef float VORTEX_ACCELERATION = 0.01  # A tiny value added to the centripetal force exerted by vortex gravities to draw in particles
cdef float VORTEX_SWALLOWDIST = 20.0  # Particles closer than this will be swallowed up and regurgitated in the bit bucket

# Fraction of radius which can go inside an object
RADIUS_PERMITTIVITY = 0.3

cdef LinearInterpolate(double val1, double val2, double t):
    cdef double diff, dist
    diff = val2 - val1
    dist = diff * t
    return val1 + dist

cdef CosineInterpolate(double val1, double val2, double t):
    cdef double amplitude, midpoint
    amplitude = (val2 - val1) / 2.0
    midpoint = (val1 + val2) / 2.0
    return (amplitude * cos(pi * (1.0 - t))) + midpoint

cdef LinearInterpolateKeyframes(curframe, key1, key2, val1, val2):
    if key1 == key2:
        return val2
    factor = float(curframe - key1) / float(key2 - key1)
    return LinearInterpolate(val1, val2, factor)

cdef CosineInterpolateKeyframes(curframe, key1, key2, val1, val2):
    if key1 == key2:
        return val2
    factor = float(curframe - key1) / float(key2 - key1)
...
class ParticleEffect:
    def __init__(self, np.ndarray display, pos, size):
    ...
    def LoadFromFile(self, infilename):
        ...
        data = XMLParser(infile.read()).Parse()
        infile.close()
